...
Sub pUploadDocument_OnLoad
	...
	If Not(KDocument Is Nothing) Then
		...
		If Not(Topic Is Nothing) Then		
			...
			'Else 
			'Topic.Replace "~USR::filename~", ufilename
			Topic.Replace "~STF::FileName~", ufilename
			'End If
			Dim resp : resp = SendDocument(ufilename & ".pdf", KDocument.FilePath, modulepart, ref)
			EKOManager.StatusMessage ("resp (should be 200) = " & resp)
		End If
	End If
End Sub

Sub pUploadDocument_OnUnload
